...
public class ArticleServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getArticleContent".equals(action)) {
			...
			if(!errorMsgs.isEmpty()) {
				RequestDispatcher failView=req.getRequestDispatcher("/fornt_end/article/listAllArticle.jsp");
				failView.forward(req, res);
				return;
			}
			req.setAttribute("list", list);
		...
		if ("article_insert".equals(action)) {
			...
			if(article_pic.length==0||article_pic==null) {
			articleVO= articleSvc.insert(login_member_id, article_title, article_content);
			}else {
				articleVO=articleSvc.insertWithPhoto(login_member_id, article_title, article_content, article_pic);
			}
